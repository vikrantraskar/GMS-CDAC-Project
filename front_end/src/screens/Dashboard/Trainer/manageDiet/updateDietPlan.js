...
const UpdateDietPlanForSpecificUser = () => {
    ...
    const validateForm = () => {
        ...
        if (currentInput.diet_id.value !== 0) {
            if (currentInput.diet_id.value) {// regex validatepassword
                currentInput.diet_id.isError = false;
            } else {
        ...
        if (currentInput.qty.value !== 0  && currentInput.qty.value > 0) {
            if (currentInput.qty.value) {// regex validatepassword
                currentInput.qty.isError = false;
            } else {
        ...
        if (currentInput.user_id.isError || currentInput.diet_id.isError || currentInput.qty.isError) {
            isNoError = false;
        } else {
            isNoError = true;
        }
    ...
    const onSubmit = () => {
        ...
        if (validateForm()) {
        axios
            .put(config.serverURL+ '/trainer/updatedietplanofSpecificMember', { user_id:inputFields.user_id.value , diet_id:inputFields.diet_id.value, qty:inputFields.qty.value }, {
                headers: {
    ...
    const fetch = () => {
      ...
      useEffect(()=> {
        fetch()
        const { user_id, diet_id } = location.state
        let currentInput = { ...inputFields }
        console.log("userID "+user_id+" diet id "+diet_id)
        currentInput.user_id.value = user_id
        currentInput.diet_id.value = diet_id
        setInputFields(currentInput)
    }, [])
    ...
    return (
        ...
        <div className="screen-wrapper">
            <div className="screen-heading">Update Diet Plan</div>
            <div className="screen-content">
                < div className="row">
                    ...
                    <div className="col">
                        <div className="form-field">
                            <div><label htmlFor="diet_id">Diet Id : </label></div>
                            <select
                name="diet_id"
                id="diet_id"
                onChange={(e) => {
                ...
                {dietList && dietList.length
                  ...
                  : null}
                             ...
                             {inputFields.diet_id.isError ?
                                <div className='error-wrapper'>
                                    {inputFields.diet_id.value === 0 ? 'diet_id is required.' : 'Please enter valid diet_id.'}
                                </div> : null}
                        </div>
                    </div>
                    ...
                    <div className="col">
                        <div className="form-field">
                            <div><label htmlFor="qty"> Quntity : </label></div>
                            <input
                               onChange={(e) => { onInputValueChange(e) }}
                                name="qty"
                                type="number"
                            ...
                            />
                             {inputFields.qty.isError ?
                                <div className='error-wrapper'>
                                    {inputFields.qty.value === 0 ? 'qty is required.' : 'Please enter valid qty.'}
                                </div> : null}
                        </div>
                    </div>
...
export default UpdateDietPlanForSpecificUser
